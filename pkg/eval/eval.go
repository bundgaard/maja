...
func myEval(root ast.Cons, env Env) (ast.Cons, error) {
	...
	for {
		switch root.Type {
		default:
			panic("unhandled type" + root.Type.String())
		case ast.Number:
			return root, nil
		case ast.Symbol:
...
func evalIf(root ast.Cons, env Env) (ast.Cons, error) {
	_ = root.List[0]            // if
	test := root.List[1]        // predicate or (= 1 1)
	consequence := root.List[2] // when #t
	alternative := root.List[3] // when #f
